...
function EventObservable(root, key, eventName){
    key = ""+key;
    this.key = key;
    this.root = root;
...
EventObservable.prototype = Object.create(SettableObservable.prototype);
canReflect.assign(EventObservable.prototype,{
    constructor: EventObservable,
    handler: function() {
		var old = this.value;
		this.value = peek(this.root, this.key);
	...
	setup: function() {
		this.bound = true;
		eventUtil.on(this.root, this.handler, "notify");
		this.value = peek(this.root, this.key);
	},
	teardown: function() {
		this.bound = false;
		eventUtil.off(this.observation, this.handler, "notify");
	},
    set: function(newVal) {
        canReflect.setKeyValue( this.root, this.key, newVal );
    },
    get: function(){
        canReflect.getKeyValue( this.root, this.key );
    }
});
...
// }, null, 5)
// ```
function SettableObservable(root, key, event) {
